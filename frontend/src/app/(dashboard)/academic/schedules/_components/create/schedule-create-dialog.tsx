        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
          <div className="space-y-3">
            <div className="grid grid-cols-2 gap-3">
              <div className="flex flex-col gap-1.5">
                <Label htmlFor="section" className={cn(errors.section && "text-red-500")}>Sección / Aula *</Label>
                <Select value={section} onValueChange={(val) => {
                  form.setValue("section", val ?? "");
                  form.clearErrors("section");
                }}>
                  <SelectTrigger id="section" className={cn(errors.section && "border-red-500 focus:ring-red-500")}>
              ...
              <div className="flex flex-col gap-1.5">
                <Label htmlFor="course" className={cn(errors.course && "text-red-500")}>Curso / Materia *</Label>
                <Select value={course} onValueChange={(val) => {
                  form.setValue("course", val ?? "");
                  form.clearErrors("course");
                }}>
                  <SelectTrigger id="course" className={cn(errors.course && "border-red-500 focus:ring-red-500")}>
            ...
            <div className="flex flex-col gap-1.5">
              <Label htmlFor="teacher" className={cn(errors.teacher && "text-red-500")}>Docente Asignado *</Label>
              <Select value={teacher} onValueChange={(val) => {
                form.setValue("teacher", val ?? "");
                form.clearErrors("teacher");
              }}>
                <SelectTrigger id="teacher" className={cn(errors.teacher && "border-red-500 focus:ring-red-500")}>
            ...
            <div className="grid grid-cols-3 gap-3">
              <div className="flex flex-col gap-1.5">
                <Label htmlFor="day" className={cn(errors.day && "text-red-500")}>Día</Label>
                <Select value={day} onValueChange={(val) => {
                  form.setValue("day", val ?? "1");
                  form.clearErrors("day");
                }}>
                  <SelectTrigger id="day" className={cn(errors.day && "border-red-500 focus:ring-red-500")}>
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    {DAYS_OF_WEEK.map((d) => (
              ...
              <div className="flex flex-col gap-1.5">
                <Label htmlFor="start" className={cn(errors.start && "text-red-500")}>Inicio (HH:mm)</Label>
                <Input
                  id="start"
                  {...form.register("start")}
                  placeholder="08:00"
              ...
              <div className="flex flex-col gap-1.5">
                <Label htmlFor="end" className={cn(errors.end && "text-red-500")}>Fin (HH:mm)</Label>
                <Input
                  id="end"
                  {...form.register("end")}
                  placeholder="09:30"
          ...
          <DialogFooter>
            ...
            <Button type="submit" disabled={isPending} className="cursor-pointer">
              {isPending ? "Guardando..." : "Asignar Clase"}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
