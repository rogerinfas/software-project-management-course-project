...
export class StaffProfileNotFoundException extends DomainException {
  constructor(idOrUserId: string) {
    super(
      `Perfil de personal con ID o ID de usuario ${idOrUserId} no encontrado`,
...
export class DuplicateStaffProfileException extends DomainException {
  constructor(userId: string) {
    super(
      `El usuario con ID ${userId} ya cuenta con un perfil de personal asignado`,
      'DUPLICATE_STAFF_PROFILE',
    );
  }
}
...
export class AttendanceRuleNotFoundException extends DomainException {
  constructor() {
    super(
      'No se encontró ninguna regla global de asistencia configurada',
