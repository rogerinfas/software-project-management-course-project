...
describe('EnrollmentController (Unit)', () => {
  ...
  describe('guardians', () => {
    it('should return paginated guardians', async () => {
      const guardian = new GuardianEntity({ id: 'g-1', dni: '12345678', name: 'G Name', phone: '987654321' });
      queryBus.execute.mockResolvedValue({ data: [guardian], meta: { total: 1 } });
    ...
    it('should create guardian', async () => {
      ...
      const result = await controller.createGuardian({ dni: '12345678', name: 'G Name', phone: '987654321' });

      expect(result).toEqual(guardian.toDto());
    });

    it('should update guardian', async () => {
      const guardian = new GuardianEntity({ id: 'g-1', dni: '12345678', name: 'G Name Updated', phone: '987654321' });
      commandBus.execute.mockResolvedValue(guardian);

      const result = await controller.updateGuardian('g-1', { dni: '12345678', name: 'G Name Updated', phone: '987654321' });
  ...
  describe('students', () => {
    it('should return paginated students', async () => {
      const student = new StudentEntity({ id: 'stud-1', code: 'c-1', firstName: 'John', lastName: 'Doe', dni: '12345678', level: EducationalLevel.PRIMARY, grade: '1', guardianId: 'g-1' });
      queryBus.execute.mockResolvedValue({ data: [student], meta: { total: 1 } });
  ...
  describe('formalize', () => {
    it('should formalize enrollment successfully', async () => {
      const enrollment = new EnrollmentEntity({ id: 'e-1', studentId: 'stud-1', year: 2026, status: 'activa', pdfUrl: 'pdf' });
  ...
  describe('documents', () => {
    it('should return paginated enrollment documents', async () => {
      const enrollment = new EnrollmentEntity({ id: 'e-1', studentId: 'stud-1', year: 2026, status: 'activa', pdfUrl: 'pdf' });
      queryBus.execute.mockResolvedValue({ data: [enrollment], meta: { total: 1 } });
