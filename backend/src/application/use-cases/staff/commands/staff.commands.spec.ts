...
describe('Staff Commands', () => {
  ...
  describe('CreateStaffProfileCommandHandler', () => {
    it('should throw DuplicateStaffProfileException if profile exists', async () => {
      ...
      await expect(handler.execute(command)).rejects.toThrow(DuplicateStaffProfileException);
    });

    it('should create staff profile successfully', async () => {
      staffRepository.findByUserId.mockResolvedValue(null);
      const created = new StaffProfileEntity({ id: 's-1', userId: 'u-1', specialty: 'Math', cvUrl: 'cv', entryTime: '08:00', exitTime: '16:00', gracePeriod: 5 });
  ...
  describe('RegisterAttendanceCommandHandler', () => {
    it('should throw StaffProfileNotFoundException if profile not found', async () => {
      ...
      const handler = new RegisterAttendanceCommandHandler(staffRepository, recordRepository, ruleRepository);
      const command = new RegisterAttendanceCommand('s-1', 'entry');

      await expect(handler.execute(command)).rejects.toThrow(StaffProfileNotFoundException);
    });
    ...
    it('should register attendance successfully', async () => {
      ...
      const record = new AttendanceRecordEntity({ id: 'r-1', staffId: 's-1', type: 'entry', timestamp, delayMinutes: 10, fineAmount: 5.0, method: 'FACIAL' });
