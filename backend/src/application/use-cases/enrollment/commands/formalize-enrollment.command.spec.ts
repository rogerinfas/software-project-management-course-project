import { FormalizeEnrollmentCommand, FormalizeEnrollmentCommandHandler } from './formalize-enrollment.command';
...
describe('FormalizeEnrollmentCommandHandler', () => {
  ...
  it('should throw SectionNotFoundException if section not found', async () => {
    ...
    const command = new FormalizeEnrollmentCommand(
      'John', 'Doe', '12345678', EducationalLevel.PRIMARY, '1', 'sec-1',
      '87654321', 'Guardian', '987654321'
    );
  ...
  it('should throw NoVacanciesAvailableException if capacity is reached', async () => {
    sectionRepository.findById.mockResolvedValue({ id: 'sec-1', capacity: 25, students: new Array(25) });
    const command = new FormalizeEnrollmentCommand(
      'John', 'Doe', '12345678', EducationalLevel.PRIMARY, '1', 'sec-1',
      '87654321', 'Guardian', '987654321'
    );

    await expect(handler.execute(command)).rejects.toThrow(NoVacanciesAvailableException);
  });

  it('should throw StudentAlreadyExistsException if student exists by DNI', async () => {
    sectionRepository.findById.mockResolvedValue({ id: 'sec-1', capacity: 25, students: [] });
    ...
    await expect(handler.execute(command)).rejects.toThrow(StudentAlreadyExistsException);
  });
  ...
  it('should throw DeudaPendienteException if guardian DNI ends with 99', async () => {
    ...
    const command = new FormalizeEnrollmentCommand(
      'John', 'Doe', '12345678', EducationalLevel.PRIMARY, '1', 'sec-1',
      '12345699', 'Guardian', '987654321'
    );

    await expect(handler.execute(command)).rejects.toThrow(DeudaPendienteException);
  });
  ...
  it('should successfully formalize enrollment', async () => {
    ...
    const enrollment = new EnrollmentEntity({ id: 'e-1', studentId: 's-1', year: 2026, status: 'activa', pdfUrl: 'url' });
