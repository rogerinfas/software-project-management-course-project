import { CreateStageCommand, CreateStageCommandHandler } from './create-stage.command';
...
describe('CreateStageCommandHandler', () => {
  ...
  it('should successfully create admission stage', async () => {
    const created = new AdmissionStageEntity({ id: 's-1', name: 'Stage 1', order: 1 });
    ...
    expect(repository.create).toHaveBeenCalledWith({ name: 'Stage 1', order: 1 });
  });
});
